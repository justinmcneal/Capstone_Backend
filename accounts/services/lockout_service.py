...
class LockoutService:
    ...
    def record_failed_attempt(customer) -> tuple:
        ...
        attempts_remaining = LockoutService.MAX_ATTEMPTS - customer.failed_login_attempts
        ...
        if customer.failed_login_attempts >= LockoutService.MAX_ATTEMPTS:
            ...
            customer.save()
            logger.warning(f"Account locked for {customer.email} after {LockoutService.MAX_ATTEMPTS} failed attempts")
            return (True, 0)
        
        customer.save()
        logger.info(f"Failed login attempt {customer.failed_login_attempts}/{LockoutService.MAX_ATTEMPTS} for {customer.email}")
