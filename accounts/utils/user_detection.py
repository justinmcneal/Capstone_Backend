...
def get_authenticated_user(request):
    ...
    user_id = getattr(user, "customer_id", None) or (
        user.get("customer_id") if hasattr(user, "get") else None
    )
    role = getattr(user, "role", "customer") or (
        user.get("role", "customer") if hasattr(user, "get") else "customer"
    )
