    ...
    def handle(self, *args, **options):
        ...
        if not no_input:
            ...
            if not super_admin and not permissions and not all_permissions:
                ...
                self.stdout.write(f'  A. All permissions')
                self.stdout.write('')
                perm_input = input('Select permissions (comma-separated numbers, A for all, or Enter for all): ').strip()
                    ...
                    for num in perm_input.split(','):
                        ...
                        if num.isdigit():
                            ...
                            if 0 <= idx < len(ADMIN_PERMISSIONS):
                                ...
                                self.stderr.write(f'  Warning: Invalid number {num}, skipping')
        ...
        if super_admin:
            ...
            if invalid:
                raise CommandError(f'Invalid permissions: {", ".join(invalid)}. Available: {", ".join(ADMIN_PERMISSIONS)}')
