...
class Verify2FAView(APIView):
    ...
    def post(self, request):
        ...
        use_backup_valid, use_backup, use_backup_error = parse_bool(
            use_backup_raw, "use_backup"
        )
        if not use_backup_valid:
            return APIResponseHelper.validation_error_response(
                {"use_backup": use_backup_error}
            )
        ...
        try:
            ...
            if initial_admin_setup:
                ...
                setup_confirmed, setup_backup_codes = (
                    TwoFactorService.confirm_2fa_setup(user, code)
                )
                if not setup_confirmed:
                    logger.warning(
                        f"Invalid 2FA setup/verification code for {user.email} ({user_type})"
                    )
                    return APIResponseHelper.error_response(
            ...
            if initial_admin_setup:
                logger.info(
                    f"2FA setup and verification completed for {user.email} ({user_type})"
                )
            else:
            ...
            response = APIResponseHelper.success_response(
                ...
                message=(
                    "2FA setup completed and login successful"
                    if initial_admin_setup
            ...
            return APIResponseHelper.success_response(
                data={
                    ...
                    "backup_codes_remaining": (
                        len(user.backup_codes) if user.backup_codes else 0
                    ),
                }
            )
