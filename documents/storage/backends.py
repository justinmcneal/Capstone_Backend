...
class S3StorageBackend(StorageBackend):
    ...
    def __init__(self):
        ...
        self.multipart_threshold = getattr(settings, 'AWS_S3_MULTIPART_THRESHOLD_BYTES', 5 * 1024 * 1024)
        self.multipart_chunksize = getattr(settings, 'AWS_S3_MULTIPART_CHUNKSIZE_BYTES', 5 * 1024 * 1024)
    ...
    def save(self, file, customer_id, document_type, original_filename):
        ...
        object_key = self._build_object_key(customer_id, document_type, original_filename)
            ...
            while True:
                ...
                try:
                    ...
                    if hasattr(file, 'temporary_file_path'):
                        ...
                        if extra_args:
                            transfer.upload_file(local_path, self.bucket_name, object_key, extra_args=extra_args)
                        else:
                            transfer.upload_file(local_path, self.bucket_name, object_key)
                    else:
                        # Fall back to client.upload_fileobj which is widely available
                        if extra_args:
                            self.s3.upload_fileobj(file, self.bucket_name, object_key, ExtraArgs=extra_args)
                        else:
                    ...
                    logger.warning("Upload attempt %d failed for %s, retrying in %.2fs: %s", attempt, object_key, sleep_time, exc)
    ...
    def get_url(self, file_path):
        ...
        try:
            ...
            logger.warning("Failed to generate presigned URL for %s: %s", file_path, exc)
    ...
    def generate_presigned_get_url(self, file_path, expires_in=None):
        ...
        try:
            ...
            logger.warning("Failed to generate presigned GET URL for %s: %s", file_path, exc)
            return None

    def generate_presigned_post(self, file_path, expires_in=None, fields=None, conditions=None):
        ...
        try:
            ...
            logger.warning("Failed to generate presigned POST for %s: %s", file_path, exc)
            return None

    def get_presigned_upload_for_new_object(self, customer_id, document_type, original_filename, expires_in=None):
