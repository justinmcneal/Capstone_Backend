...
class Command(BaseCommand):
    ...
    def _plot_training_curves(self, history, reports_dir, class_counts):
        ...
        # Plot 1: Loss
        ax1 = axes[0]
        ax1.plot(epochs_range, history['train_loss'], 'b-o', linewidth=2, markersize=6, label='Train Loss')
        ax1.plot(epochs_range, history['val_loss'], 'r-s', linewidth=2, markersize=6, label='Val Loss')
        ...
        # Plot 2: Accuracy
        ax2 = axes[1]
        ax2.plot(epochs_range, history['train_acc'], 'b-o', linewidth=2, markersize=6, label='Train Acc')
        ax2.plot(epochs_range, history['val_acc'], 'r-s', linewidth=2, markersize=6, label='Val Acc')
        ...
        ax3 = axes[2]
        ax3.plot(epochs_range, history['lr'], 'g-^', linewidth=2, markersize=6, label='Learning Rate')
        ...
        ax4.axhline(y=avg_count, color='red', linestyle='--', linewidth=1.5, alpha=0.7, label=f'Average: {avg_count:.0f}')
        ax4.legend(fontsize=11)
        
        plt.tight_layout()
        fig2.savefig(reports_dir / 'data_distribution.png', dpi=150, bbox_inches='tight')
        plt.close(fig2)
        ...
        ax5.axhline(y=10, color='orange', linestyle='--', linewidth=1.5, label='Warning (10%)', alpha=0.7)
        ax5.axhline(y=20, color='red', linestyle='--', linewidth=1.5, label='Overfitting (20%)', alpha=0.7)
