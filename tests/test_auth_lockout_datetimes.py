...
@patch("accounts.views.loan_officer_views._log_loan_officer_login_failure")
@patch("accounts.views.loan_officer_views.LoanOfficer.find_one")
def test_loan_officer_login_handles_expired_naive_utc_lockout(find_one, log_failure):
    officer = _loan_officer(_naive_utc_now() - timedelta(minutes=5))
    find_one.return_value = officer
    ...
    officer.save.assert_called_once()
    log_failure.assert_called_once()
...
def test_customer_lockout_service_resets_expired_naive_utc_lockout():
    customer = SimpleNamespace(
...
def test_successful_officer_password_reset_clears_login_lockout(
    ...
    officer.set_password.assert_called_once_with("NewPassword123!")
    assert officer.failed_login_attempts == 0
    assert officer.locked_until is None
    assert officer.must_change_password is False
    assert officer.password_reset_otp is None
    assert officer.password_reset_otp_expires is None
