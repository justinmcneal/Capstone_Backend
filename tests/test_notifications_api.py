...
def _make_request(
    user, method="get", path="/api/notifications/", data=None, query=None
):
...
class TestNotificationListView:
    def test_returns_empty_for_user_with_no_notifications(self, monkeypatch):
        ...
        user = SimpleNamespace(
            customer_id=str(customer.id), email=customer.email, role="customer"
        )
    ...
    def test_lists_notifications_owned_by_customer(self, monkeypatch):
        ...
        assert (
            response.data["data"]["notifications"][0]["notification_type"]
            == "loan_submitted"
        )
    ...
    def test_pagination_returns_correct_page_and_size(self, monkeypatch):
        ...
        request = _make_request(
            user, path="/api/notifications/", query={"page": 2, "page_size": 2}
        )
        view = NotificationListView()
        view.require_roles = lambda *a, **k: (True, user)
    ...
    def test_unread_filter_returns_only_unread(self, monkeypatch):
        ...
        request = _make_request(
            user, path="/api/notifications/", query={"unread": "true"}
        )
        view = NotificationListView()
        view.require_roles = lambda *a, **k: (True, user)
    ...
    def test_channel_filter_returns_matching_channel(self, monkeypatch):
        ...
        request = _make_request(
            user, path="/api/notifications/", query={"channel": "in_app"}
        )
        view = NotificationListView()
        view.require_roles = lambda *a, **k: (True, user)
...
class TestNotificationMarkReadView:
    def test_marks_notification_as_read(self, monkeypatch):
        ...
        request = _make_request(
            user, path=f"/api/notifications/{notif.id}/read/", method="post"
        )
    ...
    def test_returns_404_for_notification_owned_by_other_user(self, monkeypatch):
        ...
        other_user = SimpleNamespace(
            customer_id=str(other_id), email="other@example.com", role="customer"
        )
        request = _make_request(
            other_user, path=f"/api/notifications/{notif.id}/read/", method="post"
        )
        view = NotificationMarkReadView()
    ...
    def test_returns_400_for_invalid_notification_id(self, monkeypatch):
        ...
        request = _make_request(
            user, path="/api/notifications/invalid-id/read/", method="post"
        )
        view = NotificationMarkReadView()
        view.require_roles = lambda *a, **k: (True, user)
...
class TestNotificationMarkAllReadView:
    def test_marks_all_unread_as_read(self, monkeypatch):
        ...
        request = _make_request(
            user, path="/api/notifications/mark-all-read/", method="post"
        )
...
class TestNotificationDeleteView:
    def test_deletes_owned_notification(self, monkeypatch):
        ...
        request = _make_request(
            user, path=f"/api/notifications/{notif.id}/", method="delete"
        )
    ...
    def test_returns_404_for_other_users_notification(self, monkeypatch):
        ...
        request = _make_request(
            other_user, path=f"/api/notifications/{notif.id}/", method="delete"
        )
        view = NotificationDeleteView()
        view.require_roles = lambda *a, **k: (True, other_user)
    ...
    def test_legal_hold_blocks_owner_delete(self, monkeypatch):
        db = _setup_db(monkeypatch)
        ...
        notification = Notification(
            ...
            status="sent",
            legal_hold=True,
            legal_hold_reason="Active dispute",
        ).save()
        user = SimpleNamespace(
            customer_id=str(customer.id), email=customer.email, role="customer"
        )
        request = _make_request(user, method="delete")
        view = NotificationDeleteView()
        view.require_roles = lambda *args, **kwargs: (True, user)

        response = view.delete(request, notification_id=notification.id)

        assert response.status_code == 409
        assert response.data["code"] == "NOTIFICATION_LEGAL_HOLD"
        assert db["notifications"].find_one({"_id": notification._id}) is not None
...
class TestNotificationClearAllView:
    def test_deletes_all_owned_notifications(self, monkeypatch):
        ...
        request = _make_request(
            user, path="/api/notifications/clear-all/", method="delete"
        )
    ...
    def test_clear_all_retains_legal_hold_rows(self, monkeypatch):
        db = _setup_db(monkeypatch)
        customer = _create_customer(db)
        for held in (False, True):
            Notification(
                ...
                subject="Held" if held else "Deletable",
                channel="in_app",
                status="sent",
                legal_hold=held,
                legal_hold_reason="Dispute" if held else "",
            ).save()
        user = SimpleNamespace(
            customer_id=str(customer.id), email=customer.email, role="customer"
        )
        ...
        assert response.status_code == 200
        assert response.data["data"] == {"deleted_count": 1, "retained_count": 1}
        assert db["notifications"].count_documents({"legal_hold": True}) == 1
