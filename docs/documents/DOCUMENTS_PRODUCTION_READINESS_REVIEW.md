...
- [x] ~~Execute the Documents real-Mongo pagination/index and concurrent-review
  harnesses against the configured development/test cluster; both passed using
  randomly named temporary databases removed by the harness.~~
- [x] ~~Exercise Redis connectivity and a Celery control ping with a temporary
  loopback-only, no-persistence Redis and an isolated Celery queue; shut down
  both processes after the successful response.~~
- [x] ~~Run a process-only production configuration profile with S3, non-CNN
  mode, required ClamAV, Redis cache, HTTPS, HSTS, and secure cookies;
  `manage.py check --deploy` reported no issues.~~
- [ ] Validate the deployment S3 bucket/IAM workflow, private ClamAV
  clean/detected/outage behavior and signature updates, deployed encryption-key
  access/rotation, trusted proxies, throttles, sanitized logging, backup/restore,
  metrics, and alerts in the selected deployment-like environment.
...
Development validation evidence recorded on 2026-08-11:

- Stage 7 retention/operations plus malware controls passed 22 local tests.
- The two Documents real-Mongo tests passed against temporary databases,
  covering 5,000-record bounded queries/index behavior and concurrent review.
- Aggregate-only development inventory found no missing objects, expired upload
  sessions, retention gaps, legal-hold gaps, deleted-customer records, or state
  contradictions. It found one local-storage orphan. No object was deleted; the
  orphan must be reconciled before development storage is reused or migrated.
- The configured development Redis endpoint was initially offline. A temporary
  no-persistence Redis and dedicated Celery queue proved broker connectivity and
  worker control response, then shut down cleanly.
- The current `.env` remains development-oriented and was not rewritten.
  Process-only production-safe overrides passed Django's deployment check with
  zero findings.
- No isolated S3 bucket or ClamAV host was configured, so their opt-in harnesses
  remain unexecuted rather than being represented as passing.
...
`docs/documents/DOCUMENTS_TESTING_GUIDE.md` describes the current endpoint shapes and the
