...
class ProfileSummaryView(CustomerProfileAccessMixin, APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            alternative_complete = bool(
                alternative.education_level and alternative.housing_status
            )
            ...
            reupload_requested_docs = len(
                [d for d in documents if d.reupload_requested]
            )
    ...
    def _get_missing_items(
        self, personal_complete, business_complete, alternative_complete
    ):
    ...
    def _get_completed_section_names(
        self,
        personal_complete,
        business_complete,
        alternative_complete,
    ...
    def put(self, request):
        ...
        if unknown_keys:
            return error_response(
                ...
                errors={
                    "preferences": f"Unsupported keys: {', '.join(sorted(unknown_keys))}"
                },
                status_code=status.HTTP_400_BAD_REQUEST,
            )
        ...
        for key in valid_keys:
            if key in prefs:
                is_valid, parsed_value, parse_error = parse_bool(
                    prefs[key], f"preferences.{key}"
                )
                if not is_valid:
                    return error_response(
