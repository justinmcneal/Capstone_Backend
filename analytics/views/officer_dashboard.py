...
class OfficerAuditLogsView(LoanOfficerRequiredMixin, APIView):
    ...
    def get(self, request):
        ...
        base_or = [
            {'user_id': officer_id, 'user_type': 'loan_officer'}
        ]
        if assigned_ids:
            base_or.append({'resource_type': 'loan', 'resource_id': {'$in': assigned_ids}})
        ...
        if date_from or date_to:
            ...
            if date_to:
                try:
                    ...
                    ts_filter['$lte'] = date_to_obj.replace(hour=23, minute=59, second=59, microsecond=999999)
                except ValueError:
                    pass
            if ts_filter:
